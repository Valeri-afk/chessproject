...
namespace ui
{
    void Node::invalidateLayout() noexcept { if (owner_) owner_->insertLayoutQueue(this); }
    void Node::registerAnimation(Animation &animation) noexcept { if (owner_) owner_->registerAnimation(animation); }
    void Node::dispatchEventImpl(UIEvent &event, const std::type_index &eventType, NodeTree &nodeTree)
    {
        ...
        for (auto &callback : callbacks) callback(event, *this);
    }
    Node::Node() = default;
    Node::~Node() = default;
    ...
    Node *Node::hitTest(float x,float y) noexcept
    {
        ...
        const float safeX=finiteOrZero(x),safeY=finiteOrZero(y); const LayoutPosition position=getActualPosition();
