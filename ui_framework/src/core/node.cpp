...
namespace ui
{
    ...
    void Node::setVisible(bool visible)
    {
        if (visible_ == visible)
            return;
        deferLayoutMutation([visible](Node &node)
    ...
    void Node::setPosition(const LayoutPosition &position)
    {
        ...
        deferLayoutMutation([safePosition](Node &node) mutable
    ...
    {
        deferLayoutMutation([positionMode](Node &node) mutable
    ...
    void Node::setSize(const LayoutSizeValue &size)
    {
        ...
        deferLayoutMutation([safe](Node &node)
                            { node.size_ = safe; });
    }
    ...
    void Node::setMinSize(const LayoutSize &size)
    {
        ...
        deferLayoutMutation([safeSize](Node &node)
                            { node.minSize_ = safeSize; keepMaxAtLeastMin(node.minSize_, node.maxSize_); });
    }
    void Node::setMaxSize(const LayoutSize &size)
    {
        const LayoutSize safeSize = sanitizeSize(size);
        deferLayoutMutation([safeSize](Node &node)
                            { node.maxSize_ = safeSize; keepMinAtMostMax(node.minSize_, node.maxSize_); });
    }
    void Node::setMinWidth(float width)
    {
        const float safeWidth = finiteOrZero(width);
        deferLayoutMutation([safeWidth](Node &node)
    ...
    void Node::setMinHeight(float height)
    {
        ...
        deferLayoutMutation([safeHeight](Node &node)
    ...
    {
        deferLayoutMutation([value](Node &node)
    ...
    {
        deferLayoutMutation([border](Node &node)
    ...
    {
        deferLayoutMutation([overflow](Node &node)
    ...
    Node *Node::hitTest(float x, float y) noexcept
    {
        ...
        const float posX = finiteOrZero(position.x), posY = finiteOrZero(position.y);
        const float width = finiteOrZero(actualSize_.width), height = finiteOrZero(actualSize_.height);
