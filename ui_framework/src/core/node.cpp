...
namespace
{
    ...
    void keepMaxAtLeastMin(ui::LayoutSize &minSize,ui::LayoutSize &maxSize)noexcept{maxSize.width=std::max(finiteOrInfinity(maxSize.width),minSize.width);maxSize.height=std::max(finiteOrInfinity(maxSize.height),minSize.height);}void keepMinAtMostMax(ui::LayoutSize &minSize,ui::LayoutSize &maxSize)noexcept{minSize.width=std::min(finiteOrInfinity(minSize.width),maxSize.width);minSize.height=std::min(finiteOrInfinity(minSize.height),maxSize.height);}void setMinWidthValue(ui::LayoutSize &minSize,ui::LayoutSize &maxSize,float width)noexcept{minSize.width=finiteOrZero(width);keepMaxAtLeastMin(minSize,maxSize);}void setMinHeightValue(ui::LayoutSize &minSize,ui::LayoutSize &maxSize,float height)noexcept{minSize.height=finiteOrZero(height);keepMaxAtLeastMin(minSize,maxSize);}void setMaxWidthValue(ui::LayoutSize &minSize,ui::LayoutSize &maxSize,float width)noexcept{maxSize.width=finiteOrZero(width);keepMinAtMostMax(minSize,maxSize);}void setMaxHeightValue(ui::LayoutSize &minSize,ui::LayoutSize &maxSize,float height)noexcept{maxSize.height=finiteOrZero(height);keepMinAtMostMax(minSize,maxSize);}
}
namespace ui
{
    ...
    void Node::setVisible(bool visible){if(visible_==visible)return;visible_=visible;}
    ...
    void Node::setMinSize(const LayoutSize&size){minSize_=sanitizeSize(size);keepMaxAtLeastMin(minSize_,maxSize_);}
    void Node::setMaxSize(const LayoutSize&size){maxSize_=sanitizeSize(size);keepMinAtMostMax(minSize_,maxSize_);}
