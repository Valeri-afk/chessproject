...
void LayoutSystem::arrangeRecursive(Node&node,NodeTree&nodeTree){if(!node.isVisible())return;ArrangeContext ctx;ctx.contentPosition=getContentPosition(node);ctx.contentSize=getContentSize(node);ctx.arrangeChild=[this,&nodeTree](Node&child,const LayoutPosition&position,const LayoutSize&size){if(!child.isVisible())return;child.actualPosition_=position;child.actualSize_=internal::resolveFinalSize(child,size);arrangeRecursive(child,nodeTree);};node.arrange(ctx);if(auto*panel=dynamic_cast<PanelNode*>(&node)){for(size_t i=0;i<panel->getChildCount();++i){Node*child=panel->getChild(i);if(!child||!child->isVisible()||child->getPositionMode()!=PositionMode::Absolute)continue;const LayoutSize proposal=internal::resolveMeasurementProposal(*child,ctx.contentSize);const LayoutSize allocated=child->getSize().width.isValue()||child->getSize().height.isValue()?proposal:child->getDesiredSize();child->actualPosition_=ctx.contentPosition+child->getPosition();child->actualSize_=internal::resolveFinalSize(*child,allocated);arrangeRecursive(*child,nodeTree);}}}
