...
void LayoutSystem::measureRecursive(Node&node,const LayoutSize&available,NodeTree&nodeTree){if(!node.isVisible())return;const Node::Id nodeId=node.getId();const LayoutSize border=internal::resolveMeasurementProposal(node,available);const LayoutSize content=toContentSize(node,border);MeasureContext ctx;ctx.availableContentSize=content;ctx.measureChild=[this,&nodeTree](Node&child,const LayoutSize&childContent)->LayoutSize{const Node::Id id=child.getId();measureRecursive(child,toBorderBoxSize(child,childContent),nodeTree);Node*live=nodeTree.findNode(id);return live?live->getDesiredSize():LayoutSize{};};LayoutSize desired=node.measure(ctx);if(auto*panel=dynamic_cast<PanelNode*>(&node)){for(size_t i=0;i<panel->getChildCount();++i){Node*child=panel->getChild(i);if(!child||!child->isVisible()||child->getPositionMode()!=PositionMode::Absolute)continue;measureRecursive(*child,toBorderBoxSize(*child,content),nodeTree);}}Node*live=nodeTree.findNode(nodeId);if(!live)return;live->desiredSize_=toBorderBoxSize(*live,desired);}
