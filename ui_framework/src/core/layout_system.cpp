#include "../detail/layout_system.hpp"
...
namespace ui
{
    ...
    {
        if (!renderer) return false; int width=0,height=0;
        if (!SDL_GetRenderLogicalPresentation(renderer,&width,&height,nullptr) && !SDL_GetCurrentRenderOutputSize(renderer,&width,&height)) return false;
        const LayoutSize size{static_cast<float>(width),static_cast<float>(height)}; if (size==viewportSize_) return false; viewportSize_=size; return true;
    }
    ...
    {
        { NodeTree::ScopedMutationGuard guard(nodeTree); nodeTree.forEachLayoutQueue([this,&nodeTree](Node &root)
        {
            if (!root.isVisible()) return; const Node::Id rootId=root.getId(); const LayoutSize available=makeRootAvailableSize(root); measureRecursive(root,available,nodeTree);
            Node *live=nodeTree.findNode(rootId); if (!live) return; live->actualSize_=internal::resolveFinalSize(*live,available); live->actualPosition_=live->position_; arrangeRecursive(*live,nodeTree);
        }); }
        nodeTree.flushMutationQueue();
    }
    void LayoutSystem::measureRecursive(Node &node,const LayoutSize &available,NodeTree &nodeTree)
    {
        if (!node.isVisible()) return; const Node::Id nodeId=node.getId(); const LayoutSize border=internal::resolveMeasurementProposal(node,available); const LayoutSize content=toContentSize(node,border);
        LayoutSize desired{};
        if (auto *panel=dynamic_cast<StackPanelNode *>(&node))
        {
            internal::LinearMeasureContext ctx; ctx.availableSize=content;
            ctx.measureChild=[this,&nodeTree,&node](size_t index,const LayoutSize &childContent)->LayoutSize
            {
                Node *child=node.getVisibleChild(index); if (!child) return {}; const Node::Id id=child->getId(); measureRecursive(*child,toBorderBoxSize(*child,childContent),nodeTree); Node *live=nodeTree.findNode(id); return live?live->getDesiredSize():LayoutSize{};
            };
            desired=internal::measureLinearPanel(*panel,ctx);
            for(size_t i=0;i<panel->getChildCount();++i){ Node *child=panel->getChild(i); if(!child||!child->isVisible()||child->getPositionMode()!=PositionMode::Absolute) continue; measureRecursive(*child,toBorderBoxSize(*child,content),nodeTree); }
        }
        else desired=node.measureContent(content);
        Node *live=nodeTree.findNode(nodeId); if(!live) return; live->desiredSize_=toBorderBoxSize(*live,desired);
    }
    void LayoutSystem::arrangeRecursive(Node &node,NodeTree &nodeTree)
    {
        if (!node.isVisible()) return; internal::LinearArrangeContext ctx; ctx.contentPosition=getContentPosition(node); ctx.contentSize=getContentSize(node);
        if (auto *panel=dynamic_cast<StackPanelNode *>(&node))
        {
            ctx.placeChild=[this,&nodeTree,&node](size_t index,const LayoutPosition &position,const LayoutSize &size){ Node *child=node.getVisibleChild(index); if(!child||!child->isVisible()) return; child->actualPosition_=position; child->actualSize_=internal::resolveFinalSize(*child,size); arrangeRecursive(*child,nodeTree); };
            internal::arrangeLinearPanel(*panel,ctx);
            for(size_t i=0;i<panel->getChildCount();++i){ Node *child=panel->getChild(i); if(!child||!child->isVisible()||child->getPositionMode()!=PositionMode::Absolute) continue; const LayoutSize proposal=internal::resolveMeasurementProposal(*child,ctx.contentSize); const LayoutSize allocated=child->getSize().width.isValue()||child->getSize().height.isValue()?proposal:child->getDesiredSize(); child->actualPosition_=ctx.contentPosition+child->getPosition(); child->actualSize_=internal::resolveFinalSize(*child,allocated); arrangeRecursive(*child,nodeTree); }
        }
