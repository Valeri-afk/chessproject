...
namespace
{
    ...
    ui::Padding sanitizePadding(const ui::Padding &padding) noexcept { ui::Padding p=padding; p.left=std::max(0.0f,finiteOrZero(p.left)); p.right=std::max(0.0f,finiteOrZero(p.right)); p.top=std::max(0.0f,finiteOrZero(p.top)); p.bottom=std::max(0.0f,finiteOrZero(p.bottom)); return p; }
    ui::Border sanitizeBorder(const ui::Border &border) noexcept { ui::Border b=border; b.left=std::max(0.0f,finiteOrZero(b.left)); b.right=std::max(0.0f,finiteOrZero(b.right)); b.top=std::max(0.0f,finiteOrZero(b.top)); b.bottom=std::max(0.0f,finiteOrZero(b.bottom)); return b; }
...
void LayoutSystem::measureRecursive(Node&node,const LayoutSize&available,NodeTree&nodeTree){if(!node.isVisible())return;const Node::Id nodeId=node.getId();const LayoutSize border=internal::resolveMeasurementProposal(node,available);const LayoutSize content=toContentSize(node,border);MeasureContext ctx;ctx.availableContentSize=content;ctx.measureChild=[this,&nodeTree](Node&child,const LayoutSize&childContent)->LayoutSize{const Node::Id id=child.getId();measureRecursive(child,toBorderBoxSize(child,childContent),nodeTree);Node*live=nodeTree.findNode(id);return live?live->getDesiredSize():LayoutSize{};};const LayoutSize desiredContent=node.measure(ctx);if(auto*panel=dynamic_cast<PanelNode*>(&node)){for(size_t i=0;i<panel->getChildCount();++i){Node*child=panel->getChild(i);if(!child||!child->isVisible()||child->getPositionMode()!=PositionMode::Absolute)continue;measureRecursive(*child,toBorderBoxSize(*child,content),nodeTree);}}Node*live=nodeTree.findNode(nodeId);if(!live)return;const LayoutSize desiredBorder=toBorderBoxSize(*live,desiredContent);live->desiredSize_=internal::resolveFinalSize(*live,desiredBorder);}
void LayoutSystem::arrangeRecursive(Node&node,NodeTree&nodeTree){if(!node.isVisible())return;ArrangeContext ctx;ctx.contentPosition=getContentPosition(node);ctx.contentSize=getContentSize(node);ctx.desiredSize=[&nodeTree](Node&child)->LayoutSize{Node*live=nodeTree.findNode(child.getId());return live?live->getDesiredSize():LayoutSize{};};ctx.arrangeChild=[this,&nodeTree](Node&child,const LayoutPosition&position,const LayoutSize&size){if(!child.isVisible())return;child.actualPosition_=position;child.actualSize_=internal::resolveFinalSize(child,size);arrangeRecursive(child,nodeTree);};node.arrange(ctx);if(auto*panel=dynamic_cast<PanelNode*>(&node)){for(size_t i=0;i<panel->getChildCount();++i){Node*child=panel->getChild(i);if(!child||!child->isVisible()||child->getPositionMode()!=PositionMode::Absolute)continue;const LayoutSize proposal=internal::resolveMeasurementProposal(*child,ctx.contentSize);const LayoutSize allocated=child->getSize().width.isValue()||child->getSize().height.isValue()?proposal:child->getDesiredSize();child->actualPosition_=ctx.contentPosition+child->getPosition();child->actualSize_=internal::resolveFinalSize(*child,allocated);arrangeRecursive(*child,nodeTree);}}}
