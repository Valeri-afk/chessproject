...
namespace ui
{
    ...
    bool ModalSystem::closeModal(NodeTree &nodeTree, InputSystem &input)
    {
        ...
        ModalSession session = modals_.back(); modals_.pop_back();
        input.cancelPointerInteraction(nodeTree); input.clearFocus(nodeTree);
        restoreFocusAfterClose(nodeTree, input, session); updateBackdropState(); return true;
    }
    ...
    {
        for (size_t i = modals_.size(); i > 0; --i) eraseInvalidModalSession(nodeTree, input, i - 1);
        updateBackdropState();
        if (!modals_.empty()) syncFocusForTopModal(nodeTree, input);
    }
    ...
    {
        for (const Node *current = node; current; current = current->getParent()) if (current == ancestor) return true;
        return false;
    }

    void ModalSystem::restoreFocusAfterClose(NodeTree &nodeTree, InputSystem &input, const ModalSession &session) const
    {
        if (Node *previousModalFocus = findFirstFocusableInModal(nodeTree, session.previousModalId)) { focusOrClear(nodeTree, input, previousModalFocus); return; }
    ...
    bool ModalSystem::eraseInvalidModalSession(NodeTree &nodeTree, InputSystem &input, size_t index)
    {
        ...
        if (wasTop) { input.cancelPointerInteraction(nodeTree); input.syncState(nodeTree); restoreFocusAfterClose(nodeTree, input, removedSession); }
        return true;
    }
