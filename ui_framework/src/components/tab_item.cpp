...
namespace ui
{
    TabItem::TabItem() : text_(std::make_unique<TextContent>()) { setPadding({12.0f,12.0f,8.0f,8.0f}); setFocusable(true); setCapturable(true); addHandler<MouseClickEvent>([this](MouseClickEvent &event, Node &) { handleMouseClick(event); }); }
