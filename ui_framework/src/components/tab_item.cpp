...
namespace ui
{
    ...
    void TabItem::setText(std::string text)
    {
        if (text_.getText() == text)
            return;
        deferLayoutMutation([text = std::move(text)](Node &node) { static_cast<TabItem &>(node).text_.setText(text); });
    }
    ...
    void TabItem::setFont(TTF_Font *font)
    {
        if (text_.getFont() == font)
            return;
        deferLayoutMutation([font](Node &node) { static_cast<TabItem &>(node).text_.setFont(font); });
    }
    ...
    void TabItem::draw(SDL_Renderer *renderer)
    {
        ...
        if (background.a > 0)
            primitives::boxRGBA(renderer, position.x, position.y,
        ...
        text_.draw(renderer,
                   {position.x + border.left + padding.left, position.y + border.top + padding.top},
