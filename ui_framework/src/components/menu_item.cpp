...
namespace ui
{
    ...
    MenuItem::MenuItem() : textPrimitive_(std::make_unique<TextPrimitive>()) { setPadding({12.0f,12.0f,8.0f,8.0f}); setFocusable(true); setCapturable(true); addHandler<MouseEnterEvent>([this](MouseEnterEvent &e, Node &) { handleMouseEnter(e); }); addHandler<MouseLeaveEvent>([this](MouseLeaveEvent &e, Node &) { handleMouseLeave(e); }); addHandler<MouseClickEvent>([this](MouseClickEvent &e, Node &) { handleMouseClick(e); }); }
    MenuItem::~MenuItem() = default;
    void MenuItem::setText(std::string text) { if (textLayout_.getText() == text) return; textLayout_.setText(std::move(text)); }
    const std::string &MenuItem::getText() const noexcept { return textLayout_.getText(); }
    void MenuItem::setFont(TTF_Font *font) { if (textLayout_.getFont() == font) return; textLayout_.setFont(font); }
    TTF_Font *MenuItem::getFont() const noexcept { return textLayout_.getFont(); }
    ...
    LayoutSize MenuItem::measureContent(const LayoutSize &a) const { return textLayout_.measure(a.width); }
    void MenuItem::draw(SDL_Renderer *r) { if (!r || !textPrimitive_) return; const auto p=getActualPosition(); const auto s=getActualSize(); Color bg=backgroundColor_; if(highlighted_) bg=lighten(bg,0.12f); if(selected_) bg=lighten(bg,0.18f); if(!isEnabled()) bg=multiplyAlpha(bg,0.5f); if(bg.a) primitives::boxRGBA(r,p.x,p.y,p.x+s.width,p.y+s.height,bg.r,bg.g,bg.b,bg.a); const auto pad=getPadding(); const auto border=getBorder(); textPrimitive_->draw(r,textLayout_.getText(),textLayout_.getFont(),TextAlignment::START,TextAlignment::CENTER,textColor_,{p.x+border.left+pad.left,p.y+border.top+pad.top},{std::max(0.0f,s.width-border.left-border.right-pad.left-pad.right),std::max(0.0f,s.height-border.top-border.bottom-pad.top-pad.bottom)}); }
