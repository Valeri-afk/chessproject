...
namespace ui
{
    ...
    MenuItem *Menu::addItem(std::unique_ptr<MenuItem> item, size_t index) { if(!item) return nullptr; MenuItem *raw=item.get(); raw->setOnActivate([this](MenuItem &i){ handleItemActivation(i); }); return static_cast<MenuItem*>(addChild(std::move(item), index)); }
    ...
    void Menu::setOnItemActivate(ItemActivationCallback callback) { onItemActivate_=std::move(callback); }
    ...
    void Menu::handleItemActivation(MenuItem &item) { if(!item.isEnabled()) return; setActiveItem(&item); setSelectedItem(&item); if(onItemActivate_) onItemActivate_(item); }
}
