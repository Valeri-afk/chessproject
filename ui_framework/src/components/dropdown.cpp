...
namespace ui
{
    ...
    Dropdown::Dropdown() {
        auto trigger=std::make_unique<Button>(); trigger_=trigger.get(); trigger_->setText(placeholder_); trigger_->setOnActivate([this](Button &){handleTriggerActivate();});
        auto menu=std::make_unique<Menu>(); menu_=menu.get(); menu_->setVisible(false); menu_->setPositionMode(PositionMode::Absolute); menu_->setMainAlignment(MainAxisAlignment::START); menu_->setCrossAlignment(CrossAxisAlignment::STRETCH); menu_->setOnItemActivate([this](MenuItem &i){handleItemActivate(i);});
