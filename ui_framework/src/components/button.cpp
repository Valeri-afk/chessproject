...
namespace ui
{
    ...
    void Button::setText(std::string text) { if (textLayout_.getText() == text) return; textLayout_.setText(std::move(text)); }
    const std::string &Button::getText() const noexcept { return textLayout_.getText(); }
    void Button::setFont(TTF_Font *font) { if (textLayout_.getFont() == font) return; textLayout_.setFont(font); }
    TTF_Font *Button::getFont() const noexcept { return textLayout_.getFont(); }
    ...
    LayoutSize Button::measureContent(const LayoutSize &available) const { return textLayout_.measure(available.width); }
    void Button::draw(SDL_Renderer *renderer) { if(!renderer||!textPrimitive_) return; const auto p=getActualPosition(); const auto s=getActualSize(); const auto bg=presentationBackgroundColor(); const auto border=presentationBorderColor(); const auto tc=presentationTextColor(); if(variant_!=Variant::TEXT) primitives::roundedBoxRGBA(renderer,p.x,p.y,p.x+s.width,p.y+s.height,borderRadius_,bg.r,bg.g,bg.b,bg.a); if(variant_==Variant::OUTLINED) primitives::roundedRectangleRGBA(renderer,p.x,p.y,p.x+s.width,p.y+s.height,borderRadius_,border.r,border.g,border.b,border.a); textPrimitive_->draw(renderer,textLayout_.getText(),textLayout_.getFont(),TextAlignment::CENTER,TextAlignment::CENTER,tc,p,s); }
