...
namespace ui
{
    ...
    void Button::setText(std::string text) { if (text_.getText() == text) return; deferLayoutMutation([text=std::move(text)](Node &n){ static_cast<Button&>(n).text_.setText(text); }); }
    const std::string &Button::getText() const noexcept { return text_.getText(); }
    void Button::setFont(TTF_Font *font) { if (text_.getFont() == font) return; deferLayoutMutation([font](Node &n){ static_cast<Button&>(n).text_.setFont(font); }); }
    TTF_Font *Button::getFont() const noexcept { return text_.getFont(); }
    void Button::setTextColor(Color c) noexcept { text_.setColor(c); }
    ...
    Color Button::presentationTextColor() const noexcept { return text_.getColor(); }
