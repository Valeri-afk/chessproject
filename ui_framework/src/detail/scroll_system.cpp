...
namespace ui
{
    ...
    {
        const auto max = maxOffset();
        offset.x = std::clamp(offset.x, 0.0f, max.x);
        offset.y = std::clamp(offset.y, 0.0f, max.y);
    }
    ...
    bool ScrollSystem::scrollBy(Node::Id nodeId, const ScrollOffset &delta)
    {
        ...
        if (it == states_.end())
            return false;
        it->second.offset = it->second.offset + delta;
        it->second.clampOffset();
        return true;
    }
    ...
    Node *ScrollSystem::findNearestScrollableAncestor(NodeTree &nodeTree, Node *target) const noexcept
    {
        (void)nodeTree;
    ...
    bool ScrollSystem::handleWheel(NodeTree &nodeTree, float x, float y, float deltaX, float deltaY, const Node *modalRoot)
    {
        (void)x;
        (void)y;
        (void)modalRoot;
        Node *target = nullptr;
        // Hit-testing remains owned by the input/tree layer. This method only
        // applies the already-resolved wheel delta to a registered ancestor.
        target = nodeTree.getHoveredNode();
        Node *scrollNode = findNearestScrollableAncestor(nodeTree, target);
        if (!scrollNode)
            return false;
        return scrollBy(scrollNode->getId(), {deltaX, deltaY});
    }
