...
namespace ui
{
    inline SDL_Rect toSDLRect(const Node &node) noexcept
    {
        ...
        {
            const float safe = std::clamp(value, static_cast<float>(std::numeric_limits<int>::min()), static_cast<float>(std::numeric_limits<int>::max()));
