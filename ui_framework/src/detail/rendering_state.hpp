...
namespace ui
{
    inline SDL_Rect toSDLRect(const Node &node) noexcept
    {
        ...
        {
            return static_cast<int>(std::clamp(value, static_cast<float>(std::numeric_limits<int>::min()), static_cast<float>(std::numeric_limits<int>::max())));
        };

        const int x = clampInt(std::floor(left));
        const int y = clampInt(std::floor(top));
        const int right = clampInt(std::ceil(left + width));
        const int bottom = clampInt(std::ceil(top + height));
    ...
    public:
        ...
        ~RendererStateScope()
        {
            ...
            if (hasViewport_)
                SDL_SetRenderViewport(renderer_, viewportSet_ ? &viewport_ : nullptr);
            if (hasScale_)
                SDL_SetRenderScale(renderer_, scaleX_, scaleY_);
            SDL_SetRenderClipRect(renderer_, clipEnabled_ && hasClip_ ? &clip_ : nullptr);
            if (hasColor_)
    ...
    private:
        ...
        bool hasScale_ = false, hasColor_ = false, hasBlendMode_ = false, hasColorScale_ = false;
    };
}
