...
namespace ui
{
    ...
    public:
        ...
        EventHandlerId on(std::function<void(Event &, Node &)> handler)
        {
            ...
            eventHandlers_.push_back(EventHandlerRecord{token, std::type_index(typeid(Event)),
                [handler = std::move(handler)](UIEvent &event, Node &node) { handler(static_cast<Event &>(event), node); }});
            return token;
        }
        ...
        void removeEventHandler(EventHandlerId handlerId)
        {
            ...
            eventHandlers_.erase(std::remove_if(eventHandlers_.begin(), eventHandlers_.end(),
                [eventType, handlerId](const EventHandlerRecord &record) { return record.eventType == eventType && record.token == handlerId; }),
                eventHandlers_.end());
        }
