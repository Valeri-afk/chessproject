...
namespace ui
{
    ...
    public:
        UIManager(); ~UIManager();
        UIManager(const UIManager&)=delete; UIManager& operator=(const UIManager&)=delete;
        ...
        Node *addRoot(std::unique_ptr<Node> node); Node *addOverlay(std::unique_ptr<Node> node); void removeRoot(Node *node); void removeOverlay(Node *node);
        void invalidateLayout(Node &node);
        bool enableScrolling(Node &node); bool disableScrolling(Node &node); bool isScrollingEnabled(const Node &node) const noexcept;
        bool setScrollOffset(Node &node,const ScrollOffset &offset); ScrollOffset getScrollOffset(const Node &node) const noexcept; ScrollOffset getMaximumScrollOffset(const Node &node) const noexcept;
